...
function isDailyTaskDueToday(daysSinceCreation: number, repeatFrequency: number | null): boolean {
  const frequency = repeatFrequency ?? 1;
  return daysSinceCreation >= 0 && daysSinceCreation % frequency === 0;
}

function isWeeklyTaskDueToday(task: Readonly<Tables<'tasks'>>, createdAtDate: Date): boolean {
...
function isMonthlyTaskDueToday(task: Readonly<Tables<'tasks'>>, createdAtDate: Date): boolean {
...
function isYearlyTaskDueToday(task: Readonly<Tables<'tasks'>>, createdAtDate: Date): boolean {
