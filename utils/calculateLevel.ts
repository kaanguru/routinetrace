...
export function percentageUntilNextLevel(health: number, happiness: number): number {
  ...
  const percentage =
    ((totalPointsForCurrentLevel - pointsUntilNextLevel) / totalPointsForCurrentLevel) * 100;
